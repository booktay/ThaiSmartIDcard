...
def getResultPath(suffix):
    ...
    return destination_path + "/" + suffix

def convertRegisterToTis620(i):
    tis620encoding = {
        32:' ',        #  0x20 -> SPACE
...
def getPhoto(nid):
    ...
    photo_path = getResultPath(nid + ".photo.jpg")
    fphoto = open(photo_path, "wb")

    for d in PHOTO:
        response, _, _ = readMemory(d)
        if response : fphoto.write(bytearray(response))
...
def getID():
    ...
    ID = generateText(REQ_CID)
    return ID

def getEngName():
    REQ_ENG_NAME = [0x80, 0xb0, 0x00, 0x75, 0x02, 0x00, 0x64]
    full_name = generateText(REQ_ENG_NAME)
...
def getThaiName():
    ...
    full_name = generateText(REQ_THAI_NAME)
    full_name = full_name.split()
    name = {
        "TITLE": full_name[0],
        "FIRST_NAME": full_name[1],
...
def getGender():
    REQ_GENDE = [0x80, 0xb0, 0x00, 0xE1, 0x02, 0x00, 0x01]
...
def getBirthDate():
    ...
    birth = generateText(REQ_DOB)
    return birth[:4] + "/" + birth[4:6] + "/" + birth[6:]

def getAddress():
    REQ_ADDRESS = [0x80, 0xb0, 0x15, 0x79, 0x02, 0x00, 0x64]
    address = generateText(REQ_ADDRESS)
    return address

def getCardIssuedAndExpired():
