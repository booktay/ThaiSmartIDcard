...
class ThaiNIDSmartcard:
    def __init__(self):
        self.CARDSERVICE = CardRequest(timeout=1, cardType=AnyCardType()).waitforcard()

    def connectToReader(self, deviceName=""):
        self.CARDSERVICE.connection.connect()
        getDeviceName = self.CARDSERVICE.connection.getReader()

        # deviceName = "Generic USB2.0-CRW"
        if getDeviceName.lower() == deviceName.lower():
            print('CONNECTED TO ' + getDeviceName);
            return
        else:
            raise IOError("Failed to conect : " + getDeviceName)

    def checkIfThaiSmartIDCard(self):
        print("\nCheck If Thai Smart ID Card")
        # Thai Smart ID Card Frame
        SELECT = [0x00, 0xA4, 0x04, 0x00, 0x08]
        THAI_ID_CARD = [0xA0, 0x00, 0x00, 0x00, 0x54, 0x48, 0x00, 0x01]
        APDU = SELECT + THAI_ID_CARD
        
        _, status_code, _ = self.requestByAPDU(apdu=APDU)
        if status_code in [0x90, 0x61]:
            print("Your Card : Thai Smart ID Card")
            return True
        else:
            raise ValueError("Your card isn't a Thai Smart ID Card : %x" % (status_code))

    def requestByAPDU(self, apdu=[]):
        try:
            response, status_code, start_byte = self.CARDSERVICE.connection.transmit(apdu)

            if status_code in [0x90, 0x61]:
                # print("Found Data")
                return response, status_code, start_byte
            else:
                raise ValueError("No Response from an APDU Request")
        except:
            raise IOError("APDU Request Problem")
    
    def getResponseData(self, start_byte=None):
        APDU = [0X00, 0XC0, 0x00, 0x00] + [start_byte]
        response, _, _ = self.requestByAPDU(apdu=APDU)
        return response

    def convertToText(self, hexData=[]):
        if hexData != []:
            result = ''
            for i in hexData: result += self.encodeHexToTis620(i)
            result = re.sub('(\s)+', ' ', result.replace("#", " "))
            return result
        else:
            raise ValueError("Hex Data is empty")

    def encodeHexToTis620(self, hex=""):
        tis620encoding = {
            32:' ',        #  0x20 -> SPACE
        ...
        if hex in tis620encoding:
            return tis620encoding[hex]
        else:
            raise ValueError("Can't Encode : " + hex)

    def getNationalIDNumber(self):
        print("Get National ID Number")
        REQ_CID = [0x80, 0xb0, 0x00, 0x04, 0x02, 0x00, 0x0d]
        _, _, start_byte = self.requestByAPDU(apdu=REQ_CID)
        response = self.getResponseData(start_byte=start_byte)
        return self.convertToText(hexData=response)

    def getThaiName(self):
        print("Get Thai Name")
        REQ_THAI_NAME = [0x80, 0xb0, 0x00, 0x11, 0x02, 0x00, 0x64]
        _, _, start_byte = self.requestByAPDU(apdu=REQ_THAI_NAME)
        response = self.getResponseData(start_byte=start_byte)
        thai_name = self.convertToText(hexData=response).split()
        return {
            "TITLE": thai_name[0],
            "FIRST_NAME": thai_name[1],
            "LAST_NAME": thai_name[2]
        }

    def getEngName(self):
        print("Get English Name")
        REQ_ENG_NAME = [0x80, 0xb0, 0x00, 0x75, 0x02, 0x00, 0x64]
        _, _, start_byte = self.requestByAPDU(apdu=REQ_ENG_NAME)
        response = self.getResponseData(start_byte=start_byte)
        eng_name = self.convertToText(hexData=response).split()
        return {
            "TITLE": eng_name[0].upper(),
            "FIRST_NAME": eng_name[1].upper(),
            "LAST_NAME": eng_name[2].upper()
        }

    def getGender(self):
        print("Get Gender")
        REQ_GENDER = [0x80, 0xb0, 0x00, 0xE1, 0x02, 0x00, 0x01]
        _, _, start_byte = self.requestByAPDU(apdu=REQ_GENDER)
        response = self.getResponseData(start_byte=start_byte)
        gender = int(self.convertToText(hexData=response))
        ...
        if gender > 0 and gender <= len(genders):
    ...
    def getBirthDate(self):
        ...
        _, _, start_byte = self.requestByAPDU(apdu=REQ_DOB)
        response = self.getResponseData(start_byte=start_byte)
        birth = self.convertToText(hexData=response)
        return {
            "DAY": birth[6:],
            "MONTH": birth[4:6],
            "YEAR": birth[:4]
    ...
    def getAddress(self):
        ...
        _, _, start_byte = self.requestByAPDU(apdu=REQ_ADDRESS)
        response = self.getResponseData(start_byte=start_byte)
        address = self.convertToText(hexData=response)
        return address

    def getCardIssuedDate(self):
        print("Get Card Issued Date")
        REQ_ISSUE = [0x80, 0xb0, 0x01, 0x67, 0x02, 0x00, 0x08]
        _, _, start_byte = self.requestByAPDU(apdu=REQ_ISSUE)
        response = self.getResponseData(start_byte=start_byte)
        date = self.convertToText(hexData=response)
        return {
            "DAY": date[6:],
            "MONTH": date[4:6],
            "YEAR": date[:4]
    ...
    def getCardExpiredDate(self):
        print("Get Card Expired Date")
        REQ_EXPIRE = [0x80, 0xb0, 0x01, 0x6F, 0x02, 0x00, 0x08]
        _, _, start_byte = self.requestByAPDU(apdu=REQ_EXPIRE)
        response = self.getResponseData(start_byte=start_byte)
        date = self.convertToText(hexData=response)
        return {
            "DAY": date[6:],
    ...
    def getCardIssuer(self):
        print("Get Card Issuer")
        REQ_CARD_ISSUER = [0x80, 0xb0, 0x00, 0xF6, 0x02, 0x00, 0x64]
        _, _, start_byte = self.requestByAPDU(apdu=REQ_CARD_ISSUER)
        response = self.getResponseData(start_byte=start_byte)
        issuer = self.convertToText(hexData=response).strip().split("/")
        return issuer

    def getAllData(self):
        print("\nGet Datas from Card...")
        return {
            "ID": self.getNationalIDNumber(),
            "NAME": {
                "ENG": self.getEngName(),
                "THAI": self.getThaiName()
            },
            "GENDER": self.getGender(),
            "BIRTH_DATE": self.getBirthDate(),
            "ADDRESS": self.getAddress(),
            "CARD": {
                "ISSUER": self.getCardIssuer(),
                "ISSUED_DATE": self.getCardIssuedDate(),
                "EXPIRED_DATE": self.getCardExpiredDate()
            }
        }
    ...
    def getPhoto(self):
        ...
        photo = []
        for photo_apdu_line in REQ_PHOTO:
            _, _, start_byte = self.requestByAPDU(apdu=photo_apdu_line)
            photo.append(self.getResponseData(start_byte=start_byte))
        return photo
